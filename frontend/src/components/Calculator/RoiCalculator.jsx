...
import { opportunities } from '../../data/mockOpportunities'; // <--- ADICIONE ISSO
...
const RoiCalculator = () => {
  ...
  // 🔍 CORREÇÃO 1: Sanitização no Input
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    
    // Se for campo numérico, evita começar com negativo
  ...
  const handleCalculate = async (e) => {
    ...
    try {
      // 🔍 BUSCA INTELIGENTE DE ORIGEM
      // Procura nos dados mockados a primeira região que produz esse produto
      const productOrigin = opportunities.find(op => op.product === formData.product);
      
      // Se achar, usa a posição real. Se não, usa um padrão (Centro do Brasil)
      const originCoords = productOrigin ? productOrigin.position : [-15.7975, -47.8919];

      console.log(`Simulando rota de ${productOrigin?.city || 'Origem Padrão'} para ${formData.destinationName}`);

      const data = await OpportunityService.calculateROI({
        ...formData,
        originCoords: originCoords, // <--- O SEGREDO ESTÁ AQUI
          ...
          <form onSubmit={handleCalculate} className="calculator-form">
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                {/* 🔍 CORREÇÃO 3: Adicionado min="0" em todos os inputs */}
